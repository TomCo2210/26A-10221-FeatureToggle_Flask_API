...
feature_toggle_blueprints = Blueprint('feature_toggle', __name__)


# 1. create a new feature toggle item:
@feature_toggle_blueprints.route('/feature-toggle', methods=['POST'])
...
# 2. get all features from db by package name
@feature_toggle_blueprints.route('/feature-toggle/<package_name>', methods=['GET'])
def get_all_features_from_db_by_package_name(package_name):
    """
    Get  all features from db by package name
    ---
    parameters:
        - name: package_name
          in: path
    ...
    responses:
        200:
            description: List of all features for package
        404:
    ...
    if package_collection is None:
        return jsonify({'error': 'Package name not found'
        }), 404
            
    # collect all features to list:
    for feature in package_collection.find():
        all_features.append(feature)

    return jsonify(all_features), 200

# 3. Delete all feature toggles for package name
@feature_toggle_blueprints.route('/feature-toggle/<package_name>', methods=['DELETE'])
...
@feature_toggle_blueprints.route('/feature-toggle/<package_name>/<feature_id>/update-dates', methods=['PUT'])
...
# 5. Get all active feature toggles for package name
@feature_toggle_blueprints.route('/feature-toggles/<package_name>/active', methods=['GET'])
